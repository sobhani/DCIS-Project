...
cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/tissue_segmentation/20171030-TissueSegmentation/parameters-tissuesegment.txt "${1}"
FULLDIRNAMEESC=$(echo $FULLDIRNAME | sed 's_/_\\/_g') 
for s in $SAMPLES; do	
  echo "Create bsub for ${s}"
  BNAME=`basename ${s}`
  cp /mnt/scratch/users/molecpath/sraza/scripts/MyCodes/TracerX/tissue_segmentation/20171030-TissueSegmentation/header_tissue_segmentation "${FULLDIRNAME}/bsub_tissuesegment/${BNAME}_extract.bsub_tissuesegment"  
